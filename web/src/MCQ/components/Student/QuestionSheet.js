...
import { getAllMCQ, searchMCQsID } from '../../actions'
...
class QuestionSheet extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(searchMCQsID(id));
        this.props.dispatch(getAllMCQ());
    ...
    render () {
        const {mcq} = this.props.mcqReducer
                ...
                <Container maxWidth="lg" className={classes.container}>
                    <div className="page-content">
                        ...
                        <Typography align={'left'} color={'primary'} variant={'h6'} display={'block'} style={{marginLeft:"1.5%"}}>
                            ...
                            {get(mcq, 'questionSet.question.questionNumber')} &nbsp;   {get(mcq, 'questionSet.question.question')}
                            {/*{get(mcq, 'questionSet.question.question.answerOptions')}*/}
                        ...
                        <FormControl component="fieldset">
                            ...
                            <RadioGroup
                                ...
                                aria-label="gender"
                                value= {get(mcq, 'questionSet.question.answerOptions.optionNumber')}
                                onChange={(event) => this.handleChange(event)}>
                                {
                                  // mcq.questionSet.question.answerOptions.map(q => {
                                  //       return (
                                  //           <FormControlLabel
                                  //
                                  //               value={q.optionNumber}
                                  //               control={<Radio/>}
                                  //               label={q.answerBody}/>
                                  //       )
                                  //   })
                        ...
                        {
                            mcqs.map((aquest, index) => {
                                return (
                                    <>
                                        <Typography align={'left'} color={'primary'} variant={'h6'} display={'block'} style={{marginLeft:"1.5%"}}>
                                            {aquest.questionSet.question.questionNumber} {aquest.questionSet.question.question}
                                        </Typography>
                                        <br/>
                                        <FormControl component="fieldset">
                                            <br/>
                                            <RadioGroup
                                                className={classes.group}
                                                aria-label="gender"
                                                value={ aquest.questionSet.question.answerOptions.optionNumber}
                                                onChange={(event) => this.handleChange(event, index)}>
                                                {
                                                    aquest.questionSet.question.answerOptions.map(q => {
                                                        return (
                                                            <FormControlLabel
...
}))(withStyles(useStyles)(QuestionSheet))
