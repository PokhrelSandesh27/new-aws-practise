...
import {createAssignment, uploadAssignment} from '../actions'
...
class CreateAssignmentClassroom extends Component {
    ...
    createAssignment = async (e) => {
        ...
        const createAssignmentRedux = await this.props.dispatch(createAssignment(req))
        if ((createAssignmentRedux.value || undefined) && createAssignmentRedux.value.request.status === 200) {
            const assignment = createAssignmentRedux.value.data

            if (this.state.file) {
                const fileData = new FormData()
                fileData.append('file', this.state.file)
                fileData.append('classroom', assignment.classroom)
                fileData.append('subject', assignment.subject)
                const uploadAssignmentRedux = await this.props.dispatch(uploadAssignment(assignment._id, fileData))
                    ...
                    <div className="page-content">
                        <div className="row">
                            <Breadcrumbs aria-label="breadcrumb"
                                ...
                                <Typography variant={"inherit"} color="secondary">Create</Typography>
                            </Breadcrumbs>
                            <br/> <br/>
                        ...
                        <form className={classes.root} onSubmit={this.createAssignment.bind(this)}>
...
}))(withStyles(useStyles)(CreateAssignmentClassroom))
